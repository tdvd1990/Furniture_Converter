// parser.js — the single source of truth for 3D Unit Converter's
// dimension-parsing and unit-formatting logic. Loaded as a plain <script>
// in the browser (exposes window.DimParser) and via require() in Node for
// the test suite in tests/parser.test.js. Keep ALL parsing/formatting
// logic in this file — index.html should only ever call into it, never
// duplicate it.
(function (root, factory) {
  ...
  // Spelled-out units ("millimeters", "centimeters", "meters", "feet"/
  // "foot", "inch"/"inches") alongside their abbreviations. Ordering
  // within the alternation doesn't matter for correctness here — "mm"
  // can never match as a prefix of "millimeters" (second letter differs),
  // and same for "cm"/"centimeters" — so there's no ambiguity to resolve.
  var UNIT_TOKEN = '(millimeters?\\b|mm\\b|centimeters?\\b|cm\\b|feet\\b|foot\\b|ft\\b|inches\\b|inch\\b|in\\b|meters?\\b|m\\b|"|\')';
  var NUM_UNIT_RE = new RegExp('(\\d+(?:\\.\\d+)?)\\s*' + UNIT_TOKEN + '?', 'gi');

  function normalizeUnit(u) {
    u = u.toLowerCase();
    if (u === '"' || u === 'inch' || u === 'inches') return 'in';
    if (u === "'" || u === 'feet' || u === 'foot') return 'ft';
    if (u === 'millimeter' || u === 'millimeters') return 'mm';
    if (u === 'centimeter' || u === 'centimeters') return 'cm';
