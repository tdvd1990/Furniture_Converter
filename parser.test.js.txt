...
test('"meters" spelled fully (real Marketplace-listing bug: xm-glued separator too)', function () {
  assertParsed('2.15 x 1.3 xm0.576 meters', { mode: 'ordered', nums: [2.15, 1.3, 0.576], unit: 'm' });
});

test('"meter" singular, spelled fully', function () {
  assertParsed('1 meter x 1 meter x 1 meter', { mode: 'ordered', nums: [1, 1, 1], unit: 'm' });
});

test('"centimeters" spelled fully', function () {
  assertParsed('100 x 40 x 15 centimeters', { mode: 'ordered', nums: [100, 40, 15], unit: 'cm' });
});

test('"millimeters" spelled fully', function () {
  assertParsed('1000 x 400 x 150 millimeters', { mode: 'ordered', nums: [1000, 400, 150], unit: 'mm' });
});

test('"foot" singular, spelled fully', function () {
  assertParsed('5 foot x 3 foot x 2 foot', { mode: 'ordered', nums: [5, 3, 2], unit: 'ft' });
});
...
test('comma-separated, no spaces', function () {
  assertParsed('100,40,35', { mode: 'ordered', nums: [100, 40, 35], unit: null });
});

test('comma-separated with a unit on the last number', function () {
  assertParsed('100,40, 35 cm', { mode: 'ordered', nums: [100, 40, 35], unit: 'cm' });
});

test('dash-separated, ragged spacing and repeated dashes', function () {
  assertParsed('100 - 45---70', { mode: 'ordered', nums: [100, 45, 70], unit: null });
});

test('dash-separated, no spaces, unit glued to last number', function () {
  assertParsed('100-40-35cm', { mode: 'ordered', nums: [100, 40, 35], unit: 'cm' });
});
...
// ---------------------------------------------------------------------
// 7. Natural-prose adjective labels ("wide"/"deep"/"high"/"tall"/"long")
//    ARE recognized as labels, same as the noun forms — real listings use
//    adjectives at least as often. (This used to be tested the other way:
//    a real Marketplace listing — "40 cm deep, 50 cm high, and 51 cm
//    wide" — showed that skipping these as "just prose" silently produced
//    the WRONG answer, reading the numbers positionally instead of by
//    their actual labels. Recognizing them is strictly safer.)
// ---------------------------------------------------------------------
console.log('\nadjective-form labels ("wide"/"deep"/"high"/"tall"/"long")');

test('real Marketplace listing: adjectives out of order, unit repeated on each', function () {
  assertParsed('40 cm deep, 50 cm high, and 51 cm wide',
    { mode: 'labeled', w: 51, d: 40, h: 50, unit: 'cm', lengthUsedAs: null });
});

test('"wide"/"deep"/"high", comma-separated, matches Width/Depth/Height in order', function () {
  assertParsed('100 cm wide, 40cm deep, 15cm high',
    { mode: 'labeled', w: 100, d: 40, h: 15, unit: 'cm', lengthUsedAs: null });
});

test('"tall" as a Height synonym', function () {
  assertParsed('100 cm wide, 40cm deep, 15cm tall',
    { mode: 'labeled', w: 100, d: 40, h: 15, unit: 'cm', lengthUsedAs: null });
});

test('"long" as a Length stand-in (fills Depth when Width is already present)', function () {
  var raw = DimParser.normalizeQuotes('60 cm long, 40 cm wide, 30 cm tall');
  var result = DimParser.parseDims(raw);
  if (!(result && result.mode === 'labeled' && result.w === 40 && result.d === 60 && result.h === 30 && result.lengthUsedAs === 'depth')) {
    throw new Error('expected "long" to fill the Depth slot, got ' + JSON.stringify(result));
  }
});
