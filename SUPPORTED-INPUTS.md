# What 3D Unit Converter can parse
...
100,40,35                   (commas work too, any spacing)
100 - 45---70               (dashes work too — any number of them, any spacing)
100-40-35cm                 (no spaces at all, unit glued to the last number)
2.15 x 1.3 xm0.576 meters   (real Marketplace-listing glitch — an "x" and
                              a spelled-out unit got smashed together; the
                              number scan doesn't care)
```

The number scan doesn't care what character separates the numbers — `x`,
`×`, comma, dash, repeated dashes, or nothing at all, even stray letters
from a copy-paste glitch. It just looks for the next digit sequence,
wherever it is.

Recognized units, in any position: `mm`, `millimeter(s)`, `cm`,
`centimeter(s)`, `m`, `meter(s)`, `in`, `inch`, `inches`, `ft`, `foot`,
`feet`, `"`, `'`. Only the first number found with a unit sets the unit
for all three — you don't need to repeat it three times.
...
long as they're a whole word (word-boundary protected, so a stray "w"
inside an unrelated word never matches).

Natural adjective forms work too, and are treated as the same label:
`wide` = Width, `deep` = Depth, `high`/`tall` = Height, `long` = Length.
Real listings say "51 cm wide" at least as often as "Width: 51cm" — see
section 5 below.
...
## 5. Natural-language adjective labels

People describing a real object usually say "wide," "deep," "high"/"tall,"
and "long" — not the noun forms above. These are recognized as the same
label, in any order, mixed with units however the listing writes them:

```
40 cm deep, 50 cm high, and 51 cm wide
→ Width 51cm, Depth 40cm, Height 50cm (a real Marketplace listing —
  positional guessing would have gotten this completely wrong, since the
  numbers don't appear in W/D/H order)

100 cm wide, 40cm deep, 15cm tall
→ Width 100cm, Depth 40cm, Height 15cm
```

`long` behaves like `length` — it fills in Width or Depth, whichever is
missing (see section 6).

## 6. "Length" filling in for a missing Width or Depth
...
## 7. Tab-separated or pasted spec tables
...
## 8. Noisy real-world listing text
...
## 9. What does NOT parse (by design)
